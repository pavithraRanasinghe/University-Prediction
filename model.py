...
train_columns = df_encoded.drop(columns=['Recommended_Program']).columns
joblib.dump(train_columns, 'train_columns.pkl')

# Split the dataset into features and target
X = df_encoded.drop(columns=['Recommended_Program'])
y = df_encoded['Recommended_Program']
...
for i in range(len(X_test_normalized)):
    ...
    top_similar_programs = y_train.iloc[top_similar_indices].tolist()
    top_n_cosine_predictions.append(top_similar_programs)

# Example: Print the top 10 recommended programs for the first few test samples based on cosine similarity
for i, pred in enumerate(top_n_cosine_predictions[:5]):
    print(f"Sample {i+1} Top {top_n} Recommended Programs (Cosine Similarity): {pred}")

# Evaluate accuracy based on top prediction from Random Forest
y_pred_top_1 = [pred[0] for pred in predictions]
accuracy = accuracy_score(y_test, y_pred_top_1)
print("Model Accuracy (Top 1 Prediction):", accuracy)
print("Classification Report (Top 1 Prediction):\n", classification_report(y_test, y_pred_top_1))
